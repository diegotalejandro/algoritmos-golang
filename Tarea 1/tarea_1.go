...
func PalabraMasRepetida(blogs []string, Directory string) []word {
	...
	for i := 0; i < len(blogs); i++ {
		...
		temptxt = strings.Replace(temptxt, "{", " ", -1)
		temptxt = strings.Replace(temptxt, "}", " ", -1)
		temptxt = strings.Replace(temptxt, "_", " ", -1)
		temptxt = strings.Replace(temptxt, "+", " ", -1)
		temptxt = strings.Replace(temptxt, "<", " ", -1)
		temptxt = strings.Replace(temptxt, ">", " ", -1)
		temptxt = strings.Replace(temptxt, "@", " ", -1)
		...
		for strings.Join(strings.Fields(temptxt), "") != "" {
			...
			auxtemp := DeleteWord(temptxt, palabra) //----------
			if strings.Join(auxtemp, "") != "" {
				auxtemp[0] = " " + auxtemp[0]
				auxtemp[len(auxtemp)-1] = auxtemp[len(auxtemp)-1] + " "
			}
			temptxt = strings.Join(auxtemp, " ") //----------
			//temptxt = strings.Replace(temptxt, " "+palabra+" ", " ", -1)
			//var auxtemp []string
			/*for z := 0; z < len(temptxt); z++ {
				tx := strings.Fields(temptxt)
				if tx[z] != palabra {
					if z == 0 {
						tx[z] = " " + tx[z]
					}
					if z == len(temptxt) {
						tx[len(temptxt)-1] = tx[len(temptxt)-1] + " "
					}
					fmt.Println("Funciona hasta aca", z)
					auxtemp = append(auxtemp, tx[z])
				}
			}*/
			temptxt = strings.Join(auxtemp, " ")
			if strings.Index(temptxt, " "+palabra+" ") != -1 {
				fmt.Println(";;;;;;;;;;;;;;;;;;;;;;;;;;;;;no lo borro bien ;;;;;;;;;;;;;;;;;;;;")
			}
		}
		//fmt.Println(palabrastxt, ":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::")
...
func DeleteWord(x string, word string) []string {
	a := make([]string, 0)
	b := make([]string, 0)
	b = strings.Fields(x)
	c := b
...
func PalabraS(x []word, y []word, z []word) ([]word, []word, []word) {
	var PalabraSeparadora1 []word
	var PalabraSeparadora2 []word
	var PalabraSeparadora3 []word
	for i := 0; i < len(x); i++ {
		for j := 0; j < len(y); j++ {
			for k := 0; k < len(z); k++ {
				if x[i].word == y[j].word && y[j].word == z[k].word {
					if x[i].count > y[j].count+z[k].count {
						PalabraSeparadora1 = append(PalabraSeparadora1, x[i])
					}
					if y[j].count > x[i].count+z[k].count {
						PalabraSeparadora2 = append(PalabraSeparadora2, y[j])
					}
					if z[k].count > y[j].count+x[i].count {
						PalabraSeparadora3 = append(PalabraSeparadora3, z[k])
					}
	...
	return PalabraSeparadora1, PalabraSeparadora2, PalabraSeparadora3
}
...
func main() {
	...
	var PalabraSeparadora2 []word
	var PalabraSeparadora3 []word
	PalabraSeparadora1, PalabraSeparadora2, PalabraSeparadora3 = PalabraS(xd1, xd2, xd3)
	println("Palabras Separdoras entre 13 y 17 años: ")
	for i := 0; i < 20; i++ {
		println("Palabra:    ", PalabraSeparadora1[i].word, "           Cantidad:    ", PalabraSeparadora1[i].count)
	}
	println("Palabras Separdoras entre 23 y 27 años: ")
	for i := 0; i < 20; i++ {
		println("Palabra:    ", PalabraSeparadora2[i].word, "           Cantidad:    ", PalabraSeparadora2[i].count)
	}
	println("Palabras Separdoras entre 33 y 47 años: ")
	for i := 0; i < 20; i++ {
		println("Palabra:    ", PalabraSeparadora3[i].word, "           Cantidad:    ", PalabraSeparadora3[i].count)
	}
	// ----------- Read an entire file -------------------
